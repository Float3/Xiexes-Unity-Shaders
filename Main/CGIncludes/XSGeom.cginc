    ...
    void geom(triangle v2g IN[3], inout TriangleStream<g2f> tristream)
    {
        ...
        for (int j = 2; j >= 0; j--)
        {
            float4 worldPos = (mul(unity_ObjectToWorld, IN[j].vertex));
            half outlineWidthMask = tex2Dlod(_OutlineMask, float4(IN[j].uv, 0, 0));
            float3 outlineWidth = outlineWidthMask * _OutlineWidth * .01;
            outlineWidth *= min(distance(worldPos, _WorldSpaceCameraPos) * 3, 1);

            float3 vc = IN[j].color.rgb;
            ...
            vc = vc * 2 - 1;
            float3 t = mul(unity_WorldToObject, IN[j].ntb[1]);
            float3 b = mul(unity_WorldToObject, IN[j].ntb[2]);
            float3 n = mul(unity_WorldToObject, IN[j].ntb[0]);
            ...
            half3 normalDir = normalize(lerp(IN[j].normal, calcedNormal, saturate(_OutlineNormalMode)));
            float4 outlinePos = float4(IN[j].vertex + normalDir * outlineWidth, 1);
            ...
            o.uv1 = IN[j].uv1;
            o.uv2 = IN[j].uv2;
            o.color = float4(IN[j].color.rgb, 1); // store if outline in alpha channel of vertex colors | 1 = is an outline
            o.screenPos = ComputeScreenPos(o.pos);
            o.objPos = normalize(outlinePos);
                ...
                UNITY_TRANSFER_FOG(o, o.pos);
            #else
                TRANSFER_SHADOW_CASTER_NOPOS_GEOMETRY(o, o.pos, outlinePos, IN[j].ntb[0]);
            #endif
            tristream.Append(o);
        }
        tristream.RestartStrip();
